...
export const setAuthUserData = (userId, email, login, isAuth, login_failed) =>( {type:SETUSERSTATE, payload : {userId, email, login, isAuth, login_failed} }) ; 
...
export const singInProcessCheck = () => {
    return (dispatch) => {
       ...
       return authAPI.getLoginData().then(response => {
...
export const singInLogin = (values) => {
    return (dispatch) => {
       ...
       loginAPI.signInData(values).then(response => {
        if(response.data.resultCode === 0) {
...
export const singOutLogin = () => {
    ...
    return (dispatch) => {
       loginAPI.signOutData().then(response => {
        if(response.data.resultCode === 0) {
            // dispatch(setLogin(response.data.userId)); 
            dispatch(setAuthUserData(null, null, null, false, null));
        } else {
            
            alert("No sign out");  console.log("no sign out") }
       })
    }
};
